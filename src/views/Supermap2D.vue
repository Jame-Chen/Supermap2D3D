...
<script>
import Index2D from './onemap/Index2D.vue'
...
export default {
  ...
  methods: {
    ...
    toggleComponent(lonLat, zoom) {
      if (this.activeName === 'Index3D') this.activeName = 'Index3D'
    ...
    initConfig() {
      ...
      if (config.isuser) {
        ...
        this.$http.get(config.layer).then((res) => {
          ...
          console.log('config.facility', config.facility);
          this.$http.get(config.facility).then((res) => {
            console.log('res', res);
            let configJson1 = JSON.parse(res.result.configJson)
    ...
    initProps() {
      ...
      this.baseProps['url_fileUpLoad'] = config.url_fileUpLoad
      this.baseProps['src_fileUpLoad'] = config.src_fileUpLoad
      ...
      for (let i = 0; i < layers.length; i++) {
        ...
        if (layers[i].type === 'sheshi') {
          tempLayers3 = layers[i]
          for (let j = 0; j < layers[i].children.length; j++) {
            if (layers[i].children[j].visible) this.tempUnCheckedKeys.push(layers[i].children[j].typeName)
          }
        }
      }
      this.baseProps['layers'] = tempLayers1.concat(tempLayers2).concat(tempLayers3)
      this.initLayers(tempLayers1.concat(tempLayers2).concat(tempLayers3))
      if (this.isInitLayers) {
        this.initVisibleLayers(tempLayers1.concat(tempLayers2).concat(tempLayers3))
        let temp = []
    ...
    initVisibleLayers(layers, parentLabel) {
      for (let i = 0; i < layers.length; i++) {
        ...
        if (layers[i].typeName !== 'guihua' && layers[i].typeName !== 'sheshi') {
          if (parentLabel === undefined && i === 0) this.defaultExpandedKeys.push(layers[i].typeName)
        }
        if (layers[i].hasOwnProperty('children') && layers[i].children.length > 0) {
...
/deep/ .onemap .leaflet-popup-content {
  ...
  .mapPopTitle {
    display: block;
    background: url("/src/assets/images/contentTitleR.png") repeat-y;
