  ...
  <el-container v-if="state" class="border">
    <el-main class="inborder">
      ...
      <div class="padding-10">
        <div class="tip">请在地图上点选起点与终点设施（井）后点击查询</div>
        <el-form ref="spaceSectionAnalysisForm" :model="form" label-width="104px" style="padding-top: 8px;">
          <el-form-item label="起点设施名称">
            <el-input size="small" v-model="form.node1" :readonly="readonly" disabled />
          </el-form-item>
          <el-form-item label="终点设施名称">
            <el-input size="small" v-model="form.node2" :readonly="readonly" disabled />
          </el-form-item>
...
export default {
  ...
  computed: {
    disabled () {
      if (this.form.key !== '') {
        if (this.form.key.split(',').length > 1) return false
        else return true
      } else return true
    }
  },
  methods: {
    ...
    onSearchQuery () {
      this.$refs.spaceSectionAnalysisForm.validate((valid) => {
        if (valid) {
          this.$emit('clearData')
    ...
    pathAnalystProcess () {
      // 创建最佳路径分析服务实例
      let findPathService
      if (this.analysisType === 'duanmian') findPathService = L.supermap.networkAnalystService(this.baseProps['url_transport_analyst'])
      else if (this.analysisType === 'liantong') findPathService = L.supermap.networkAnalystService(this.baseProps['url_transport_analyst_rule'])
      // 创建最佳路径分析参数实例
      ...
      findPathService.findPath(findPathParameter, (serviceResult) => {
        ...
        if (serviceResult.hasOwnProperty('error')) {
          ...
          this.$emit('setDrawOnclickFlag', true)
          this.$emit('openMessage', '选定设施间不连通!', 'error')
          this.$emit('clearDataShow')
        } else {
          let result = serviceResult.result
          if (result) {
            if (this.analysisType === 'liantong') this.$emit('openMessage', '选定设施间可连通!', 'success')
            let nodeFeatures = result.pathList[0].nodeFeatures.features
            let edgeFeatures = result.pathList[0].edgeFeatures.features
              ...
              for (let i = 0; i < layerVisible.length; i++) {
                if (layerVisible[i].tabQuery && layerVisible[i].visible) {
                  ...
                  if (typeKeyWords !== '' && feature.properties.hasOwnProperty(typeKeyWords)) {
                    if (feature.properties[typeKeyWords] === value) {
